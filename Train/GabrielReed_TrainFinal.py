...
class LegoTool:
    def __init__(self):
        ...
        self.redShader = self.createShader(0.710, 0.042, 0.042, name="LegoRed")
        self.tanShader = self.createShader(0.54, 0.453, 0.29, name="LegoTan")
        self.yellowShader = self.createShader(0.814, 0.625, 0.016, name="LegoYellow")
        self.darkGreenShader = self.createShader(0.018, 0.077, 0.012, name="LegoDarkGreen")
        self.blackShader = self.createShader(0.017, 0.017, 0.017, name="LegoBlack")
        self.brownShader = self.createShader(0.154, 0.084, 0.021, name="LegoBrown")
        self.darkGrayShader = self.createShader(0.193, 0.193, 0.193, name="LegoDarkGray")
        self.greenShader = self.createShader(0.038, 0.193, 0.079, name="LegoGreen")
        self.lightGrayShader = self.createShader(0.384, 0.384, 0.384, name="LegoLightGray")
        self.blueShader = self.createShader(0.017, 0.036, 0.292, name="LegoBlue")

        self.colors = {
            "red": self.redShader,
            "tan": self.tanShader,
            "yellow": self.yellowShader,
            "darkGreen": self.darkGreenShader,
            "black": self.blackShader,
            "brown": self.brownShader,
            "darkGray": self.darkGrayShader,
            "green": self.greenShader,
            "lightGray": self.lightGrayShader,
            "blue": self.blueShader
        }
    ...
    def initShaders(self):
        # FIXME: Copy and paste all these into the constructor as well
        self.redShader = self.createShader(0.710, 0.042, 0.042, name="LegoRed")
        self.tanShader = self.createShader(0.54, 0.453, 0.29, name="LegoTan")
        self.yellowShader = self.createShader(0.814, 0.625, 0.016, name="LegoYellow")
        self.darkGreenShader = self.createShader(0.018, 0.077, 0.012, name="LegoDarkGreen")
    ...
    def createLegoPlant(self, randomRotate=True, plantColorName = "green"):
        plantColor = self.colors[plantColorName]
        ...
        self.applyShader(plantColor, name)
        return name

    def createLegoCactus(self, height=5, small=False, plantColorName = "green"):
        plantColor = self.colors[plantColorName]
        ...
        if small:
            ...
            for i in range(height):
                # Arms
                if (i + 2) % 3 == 0:
                    cactus.append(self.createLegoBrick(1, 2, flat=True))
                    self.moveLego(0.5, i + (numFlats) / 3, 0)
                    numFlats += 1
                    createExtra = random.uniform(0, 1)
                    if createExtra > 0.5:
                        cactus.append(self.createLegoCylinder(bottom=False))
                        self.moveLego(1, i + (numFlats) / 3, 0)
                        cactus.append(self.createLegoCylinder(bottom=False, flat=True))
            ...
            name = mc.group(cactus, n="Cactus_#")
            self.applyShader(plantColor, name)
            return name
        else:
            # Sides
            ...
            # Top and bottom
            cactus.append(self.createLegoCylinder(flat=True, bottom=False))
            cactus.append(self.createLegoCylinder(flat=True, bottom=False))
            ...
            if height > 4:
                ...
                mc.polyBevel(o=0.02)
                mc.rotate(0, 0, 90)
                self.moveLego(-1.6, int(2*height/3), 0)

                cactus.append(self.createLegoBrick(1, 2, flat=True, top=True, bottom=True))
                mc.rotate(0, 90, 90)
                self.moveLego(-2.6 + 1/3, int(2*height/3) + 0.96, 0)

                cactus.append(self.createLegoBrick(1, 2, flat=True, top=False, bottom=True))
                mc.rotate(0, 90, 90)
                self.moveLego(-2.6 + 0.85, int(2*height/3) + 1.05, 0)
                ...
                mc.polyBevel(o=0.02)
                mc.rotate(180, 0, 90)
                self.moveLego(1.6, int(height/3), 0)

                cactus.append(self.createLegoBrick(1, 2, flat=True, top=True, bottom=True))
                mc.rotate(180, 90, 90)
                self.moveLego(2.6 - 1/3, int(height/3) + 0.96, 0)

                cactus.append(self.createLegoBrick(1, 2, flat=True, top=False, bottom=True))
                mc.rotate(180, 90, 90)
                self.moveLego(2.6 - 0.85, int(height/3) + 1.05, 0)
    ...
    def createTracks(self, length=5, trackColorName = "darkGray"):
        trackColor = self.colors[trackColorName]
        ...
        name = mc.group(tracks, n="Train Tracks #")
        self.applyShader(trackColor, name)
        return name
    ...
    def createFence(self, length=9):
        ...
        # Group
        name = mc.group(fence, n="Fence_#")
        mc.rotate(-90, 0, 0, r=True)
        self.applyShader(self.brownShader, name)
        return name

    def createWheel(self):
        mainColor = self.lightGrayShader

        wheelList = []
        wheel = mc.polyCylinder(h=0.4, sx=14, r=2.3 / 2)[0]
        wheelList.append(wheel)
        self.applyShader(mainColor, wheelList[len(wheelList) - 1])
    ...
    def createWheelSet(self, wheelScale=1.0):
        ...
        name = mc.group(wheelSet, n="wheelSet#")
        self.applyShader(mainColor, name)
        return name
    ...
    def createDoubleWheelSet(self):
        ...
        name = mc.group(set, n="DoubleWheelSet_#")
        self.applyShader(self.lightGrayShader, name)
        return name

    def createSteamEngine(self, mainColorName = "darkGray", accentColor1Name = "red", accentColor2Name = "lightGray"):
        mainColor = self.colors[mainColorName]
        accentColor1 = self.colors[accentColor1Name]
        accentColor2 = self.colors[accentColor2Name]

        # Steam engine
        car = []
        # Cabin
        car.append(self.createLegoBrick(8, 8, flat=True, bottom=False))
        self.applyShader(accentColor1, car[len(car) - 1])
        ...
        for i in range(2):
            ...
            for j in range(2):
                ...
                self.applyShader(mainColor, car[len(car) - 1])
        ...
        for i in range(4):
            ...
            self.applyShader(accentColor2, car[len(car) - 1])
        ...
        for i in range(2):
            ...
            mc.rotate(0, 0, 90)
            self.applyShader(self.yellowShader, car[len(car) - 1])
        ...
        return mc.group(car, n="Steam Engine")

    def createPassengerCar(self, mainColorName = "yellow", accentColorName = "red", woodColorName = "brown"):
        mainColor = self.colors[mainColorName]
        accentColor = self.colors[accentColorName]
        woodColor = self.colors[woodColorName]
        ...
        self.applyShader(woodColor, car[len(car) - 1])
        ...
        self.applyShader(accentColor, car[len(car) - 1])
        car.append(self.createLegoBrick(1, 14, bottom=False, top=False))
        self.moveLego(0, 2, -4.5)
        self.applyShader(mainColor, car[len(car) - 1])
        car.append(self.createLegoBrick(1, 16, bottom=False, top=False, flat=True))
        ...
        for i in range(4):
            ...
            self.applyShader(self.lightGrayShader, car[len(car) - 1])
    ...
    def createCoalCar(self, makeCoal=True, mainColorName = "darkGray", accentColorName = "red"):
        mainColor = self.colors[mainColorName]
        accentColor = self.colors[accentColorName]
        ...
        for i in range(2):
            ...
            if i > 1:
                side = -1
            car.append(self.createLegoBrick(2, 16, top=False, bottom=False, flat=True))
            self.moveLego(0, 5, 5*front)
            self.applyShader(accentColor, car[len(car) - 1])

            car.append(self.createLegoBrick(8, 2, top=False, bottom=False, flat=True))
            self.moveLego(7*front, 5, 0)
            self.applyShader(accentColor, car[len(car) - 1])
        ...
        if makeCoal:
            ...
            for i in range(6):
                for j in range(4):
                    rotJitter = random.uniform(-30, 30)
                    coal.append(self.createLegoCylinder(flat=True, bottom=False))
                    self.moveLego(i - 2.5, 5 + 2 / 3, j - 1.5)
                    mc.rotate(rotJitter)
                    self.applyShader(self.blackShader, coal[len(coal) - 1])
